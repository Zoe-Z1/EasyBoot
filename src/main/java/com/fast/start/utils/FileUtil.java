...
public class FileUtil extends cn.hutool.core.io.FileUtil {
     ...
     * @param fastFile 文件配置
     */
    public static void checkFile(MultipartFile file, FastFile fastFile) {
        ...
        String[] fileTypes = fastFile.getFileType().split(",");
        // 校验文件拓展名
        ...
        // 校验文件大小
        checkSize(file.getSize(), fastFile.getMaxFileSize());
    }
     ...
     * @param fastFile 文件配置
     */
    public static void checkImage(MultipartFile file, FastFile fastFile) {
        if (file == null || file.isEmpty()) {
        ...
        String[] imageTypes = fastFile.getImageType().split(",");
        // 校验图片拓展名
        ...
        // 校验图片大小
        checkSize(file.getSize(), fastFile.getMaxImageSize());
    }
     ...
     * @param path 路径
     */
    public static void checkPath(String path, FastFile fastFile) {
        ...
        // 任务文件下载漏洞防护
        try {
            String canonicalFilePath = new File(fastFile.getFilePath()).getCanonicalPath();
            String canonicalImagePath = new File(fastFile.getImagePath()).getCanonicalPath();
     ...
     * @return
     */
    public static String inverseAnalysis(String path, FastFile fastFile) {
        path = path.substring(path.indexOf("/"));
        if (path.startsWith(fastFile.getFileMapPath())) {
            return path.replace(fastFile.getFileMapPath(), fastFile.getFilePath());
        } else if (path.startsWith(fastFile.getImageMapPath())) {
            return path.replace(fastFile.getImageMapPath(), fastFile.getImagePath());
        }
