package com.fast.start.admin.scheduledTask.service.impl;

import cn.hutool.core.util.StrUtil;
import com.baomidou.mybatisplus.core.metadata.IPage;
import com.baomidou.mybatisplus.extension.plugins.pagination.Page;
import com.fast.start.admin.scheduledTask.entity.*;
import com.fast.start.admin.scheduledTask.mapper.ScheduledTaskMapper;
...
import com.fast.start.common.quartz.FastJobManager;
...
public class ScheduledTaskServiceImpl extends ServiceImpl<ScheduledTaskMapper, ScheduledTask> implements IScheduledTaskService {
    ...
    public void change(ChangeJobDTO dto) {
        ...
        if (status) {
            if (task.getJobStatus() == 1) {
                fastJobManager.resumeJob(task);
            } else {
                fastJobManager.pauseJob(task);
            }
        }
    }
    ...
    public Boolean create(ScheduledTaskCreateDTO dto) {
        ...
        if (status) {
            fastJobManager.createJob(entity);
        }
        return status;
    }
    ...
    public Boolean deleteById(Long id) {
        ...
        if (status) {
            status = fastJobManager.deleteJob(task);
        }
        return status;
    }
    ...
    public Boolean deleteBatchByIds(List<Long> ids) {
        ...
        if (status) {
            status = fastJobManager.deleteJobs(tasks);
        }
        return status;
    }
